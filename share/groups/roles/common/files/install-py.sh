...
If a Python 3.x interpreter does not exists at PATH (default:
...
if ! [ -x "$python" ]; then
    ...
    if [ -f /etc/os-release ]; then
        ...
        die $EX_UNAVAILABLE "Unsupported OS - cannot install Python 3"
    fi
    ...
    case "$os" in
        debian|ubuntu)
            ...
            apt-get update
            do_or_die env DEBIAN_FRONTEND=noninteractive apt-get install -yq python3
            ;;
        redhat|centos)
            case "$ver" in
                7*) do_or_die yum install -y python3 ;;
                6*) do_or_die yum install -y python3 ;;
            esac
            ;;
    esac
