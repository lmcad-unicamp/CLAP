...
class SQLiteRepository(AbstractRepository):
    """ Repository implementation using the sqlite, document-oriented database.
    """

    __repository_id__ = 'sqlite'
    __repository_name__ = 'SQLite Repository'
    __repository_version__ = '0.1.0'
    ...
    def __init__(self, repository: str, create_repository: bool, storage_type: str = None, *args, **kwargs):
        ...
        :param kwargs: Additional keyword arguments to be used
        """
        super(SQLiteRepository, self).__init__(repository, create_repository, storage_type)
        self.repository = repository
        self.create_repository = create_repository
    ...
    def retrieve_tables(self) -> List[str]:
        ...
        :rtype: List[str]
        """
        return self.sqlite_obj.get_tablenames(self.repository)
    ...
    def exists_table(self, table: str, *args, **kwargs) -> bool:
        ...
        :rtype: bool
        """
        return table in self.sqlite_obj.get_tablenames(self.repository)
    ...
    def retrieve_elements(self, table: str, cast_to: type, **where) -> List[Any]:
        ...
        self.__set_table__(table)
        if not where:
            return [cast_to(**db_obj) for db_obj in self.sqlite_obj.values()]
        values = []
        for db_obj in self.sqlite_obj.values():
            for k, v in where.items():
                try:
                    if db_obj[k] == v:
                        values.append(cast_to(**db_obj))
                except Exception:
                    pass
        return values
