...
class Query(ABC):
    @abstractmethod
    def query(self, metrics: List[str] = None) -> Dict[str, List[Value]]:
        pass
...
class SpitsQuery(Query):
    def __init__(self, cluster_id: str, jobid: str, pypits_path: str = None, spits_jobs_path: str = None):
...
class HeuristicModule(AbstractModule):
    @staticmethod
    def get_module(**defaults_override) -> 'HeuristicModule':
        heuristic = defaults_override.get('heuristic')
        return HeuristicModule(heuristic)
    ...
    def run(self):
        ...
        # query = self.query_mod
        #
        # for i in range(self.max_retries):
        #     try:
        #         print(f'Querying metrics ({i+1}/{self.max_retries})....')
        #         ret_val = query.query(metrics)
        #         self.metrics_queries.append((ret_val, time.time()))
        #         to_print = dict()
        #         for metric_name, values in ret_val.items():
        #             v = [asdict(x) for x in values]
        #             to_print[metric_name] = v
        #
        #         print(f"Got values: {yaml.dump(to_print, sort_keys=True)}")
        #         print()
        #     except Exception as e:
        #         logger.error(e)
        #
        #     print(f"Sleeping for {self.query_interval} seconds....")
        #     time.sleep(self.query_interval)
        #
        # print("Metrics final...")
        # for ret_val, obtain_time in self.metrics_queries:
        #     to_print = dict()
        #     for metric_name, values in ret_val.items():
        #         v = [asdict(x) for x in values]
        #         to_print[metric_name] = v
